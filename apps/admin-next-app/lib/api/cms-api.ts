...
export const cmsApi = createApi({
  ...
  baseQuery: fetchBaseQuery({
    baseUrl: '/app/api/cms', // Proxy endpoint
  ...
  endpoints: (builder) => ({
    ...
    getVendorBlockByUIBlock: builder.query({
      query: ({ blockId, themeId, screenId }: { blockId: string; themeId?: string; screenId?: string }) => ({
        url: '',
        method: 'POST',
        body: {
    ...
    getBlockDefinitions: builder.query({
      query: ({ themeId, screenName }: { themeId: string; screenName: string }) => ({
        url: '',
        method: 'POST',
        body: {
